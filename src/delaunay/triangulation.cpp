...
// int Triangulation::makeTri(int pIndex1, int pIndex2, int pIndex3, bool autoMerge)
int Triangulation::makeTri(int pIndex1, int pIndex2, int pIndex3, bool) {
	// Add a triangle into the Triangulation with the three point indices
...
void Triangulation::delTri(OrTri ef) {
	...
	//  keep OrTri values outside of Triangulation.
...
OrTri Triangulation::enext(OrTri ef) {
	int tId = ef >> 3;
...
OrTri Triangulation::sym(OrTri ef) {
	// sym cycles  i <> i + 3 % 6
	int tId = ef >> 3;
	int v = ef & 7; // ef & 00111b ... last 3 bits.
...
OrTri Triangulation::fnext(OrTri ef) {
	int tId = ef >> 3;
	int v = ef & 7; // ef & 00111b ... last 3 bits.
...
void Triangulation::getVertexIdx(OrTri ef, int& pIdx1, int& pIdx2, int& pIdx3) {
...
int Triangulation::org(OrTri ef) {
...
int Triangulation::dest(OrTri ef) {
...
void Triangulation::fmerge(OrTri abc, OrTri abd) {
...
void Triangulation::relableRefsToTri(OrTri oldEF, OrTri newEF) {
...
void Triangulation::fdetach(OrTri abc) {
...
void Triangulation::eraseAllTriangles() {
	triPoints.clear();
}



void Triangulation::incidentTriangles(int ptIndex, int& noOrTri, OrTri* otList) {
